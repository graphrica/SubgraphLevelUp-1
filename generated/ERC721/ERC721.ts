...
export class ERC721 extends ethereum.SmartContract {
  ...
  mintItem(to: Address, tokenURI: string): BigInt {
    let result = super.call("mintItem", "mintItem(address,string):(uint256)", [
  ...
  try_mintItem(to: Address, tokenURI: string): ethereum.CallResult<BigInt> {
    let result = super.tryCall(
      "mintItem",
      "mintItem(address,string):(uint256)",
      [ethereum.Value.fromAddress(to), ethereum.Value.fromString(tokenURI)]
    );
    if (result.reverted) {
      return new ethereum.CallResult();
    }
